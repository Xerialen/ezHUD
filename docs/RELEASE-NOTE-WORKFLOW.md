...
6. **Private draft.** Prepare one private, one-message `#outbox` draft from the canonical JSON payload, and **do not post it**. `#outbox` is the owner-review Discord channel and the only destination; the payload is prepared with `posted: false`. Link previews are suppressed and any URL is wrapped in `<…>`; mentions are disabled, with no `@everyone`/`@here`/user syntax in the text. Attachments are local files uploaded with the message, never links, each under the exact name declared in `attachments`. **This repository contains no Discord posting path and must not gain one** — the prepared payload is handed to Terra for the owner-review post, and the owner forwards it onward manually. Do not redesign the message while posting.
